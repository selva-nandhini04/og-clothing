-- ============================================
-- OG CLOTHING — Supabase Database Setup SQL
...
create policy "Anyone can enter lottery" on lottery_entries for insert with check (true);

-- RPC for incrementing product views safely
...
values
  ...
  ('Vanguard Technical Parka', 'vanguard-technical-parka', 'Storm-grade technical parka with sealed seams and oversized hood.', 1899, 2799, 1599, 'Core / 001', 'Outerwear', ARRAY['S','M','L','XL','XXL'], true, 100),
  ('OG Stadium Jersey', 'og-stadium-jersey', 'Match-day performance jersey. Moisture-wicking mesh, OG embroidery on chest.', 1199, 1699, 999, 'FIFA''26 / 01', 'Football', ARRAY['XS','S','M','L','XL','XXL'], true, 500),
  ...
  ('OG Coach Jacket', 'og-coach-jacket', 'Wind-resistant coach jacket. Chest patch pocket, ribbed collar and cuffs.', 1699, 2299, 1399, 'Core / 003', 'Outerwear', ARRAY['XS','S','M','L','XL','XXL'], true, 200),
  ('Utility Cargo Short', 'utility-cargo-short', 'Relaxed fit cargo short. Multiple utility pockets. Adjustable waistband.', 899, 1299, 749, 'Utility / 05', 'Denim', ARRAY['XS','S','M','L','XL','XXL'], true, 150)
on conflict (slug) do nothing;

insert into collections (name, slug, description, tag)
values
  ('Oversized Archive', 'oversized-archive', 'The foundation. Boxy silhouettes, 500gsm loop-back cotton, garment-dyed in-house. Every piece is numbered in a run of 200.', 'Core · 24 pieces · Vol. 01'),
  ('Football Terrace', 'football-terrace', 'Designed for the terraces. Match-day energy, off-pitch wearability. Co-engineered with stadium architects and retired strikers.', 'FIFA''26 Capsule · 12 pieces'),
  ('Spring / Winter 2026', 'spring-winter-2026', 'The debut seasonal collection. Hand-finished stitching, dead-of-night silhouettes, and brutalist construction throughout.', 'Seasonal · Vol. 01')
on conflict (slug) do nothing;
